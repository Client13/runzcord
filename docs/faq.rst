...
This is a list of Frequently Asked Questions regarding using ``runzcord`` and its extension modules. Feel free to suggest a
...
Example code can be found in the `examples folder <https://github.com/runzcordDev/runzcord/tree/master/examples>`_
in the repository.
...
Putting both of these pieces of info together, you get the following: ::
    ...
    client = runzcord.Client(activity=runzcord.Game(name='my game'))

    # or, for watching:
    activity = runzcord.Activity(name='my activity', type=runzcord.ActivityType.watching)
    client = runzcord.Client(activity=activity)
...
If you want to upload an image it's as simple as: ::
    ...
    await channel.send(file=runzcord.File('my_file.png'))

If you have a file-like object you can do as follows: ::

    with open('my_file.png', 'rb') as fp:
        await channel.send(file=runzcord.File(fp, 'new_filename.png'))

To upload multiple files, you can use the ``files`` keyword argument instead of ``file``\: ::

    my_files = [
        runzcord.File('result.zip'),
        runzcord.File('teaser_graph.png'),
    ]
...
.. code-block:: python3
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(my_url) as resp:
            ...
            await channel.send(file=runzcord.File(data, 'cool_image.png'))
...
Quick example: ::
    ...
    # no ID, do a lookup
    emoji = runzcord.utils.get(guild.emojis, name='LUL')
    if emoji:
        await message.add_reaction(emoji)
...
this together we can do the following: ::
    ...
    voice.play(runzcord.FFmpegPCMAudio(url), after=my_after)

How do I run something in the background?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

`Check the background_task.py example. <https://github.com/runzcordDev/runzcord/blob/master/examples/background_task.py>`_
...
Quick example: ::
    ...
    guild = runzcord.utils.get(client.guilds, name='My Server')

    # make sure to check if it's found
    if guild is not None:
        # find a channel by name
        channel = runzcord.utils.get(guild.text_channels, name='cool-channel')
...
Quick example: ::
    ...
    file = runzcord.File("path/to/my/image.png", filename="image.png")
    embed = runzcord.Embed()
...
Questions regarding ``runzcord.ext.commands`` belong here.
