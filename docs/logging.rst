...
*runzcord* logs errors and debug information via the :mod:`logging` python
...
runzcord as well as other libraries that use the ``logging`` module
...
example to write the logs to a file called ``runzcord.log`` instead of
outputting them to the console the following snippet can be used::

    import runzcord
    import logging

    logger = logging.getLogger('runzcord')
    logger.setLevel(logging.DEBUG)
    handler = logging.FileHandler(filename='runzcord.log', encoding='utf-8', mode='w')
