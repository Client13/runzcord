...
So to check if something is a guild channel you would do: ::
    ...
    isinstance(channel, runzcord.abc.GuildChannel)

And to check if it's a private channel you would do: ::

    isinstance(channel, runzcord.abc.PrivateChannel)

Of course, if you're looking for only a specific type you can pass that too, e.g. ::

    isinstance(channel, runzcord.TextChannel)
...
- ``Channel.is_private``
    ...
    - e.g. ``isinstance(channel, runzcord.abc.GuildChannel)`` will check if it isn't a private channel.
...
- :class:`Attachment` to represent a runzcord attachment.
...
file attachments, you must now use a :class:`File` pseudo-namedtuple to upload a single file. ::
    ...
    # after
    await channel.send('Hello', file=runzcord.File('cool.png', 'testing.png'))

This change was to facilitate multiple file uploads: ::

    my_files = [
        runzcord.File('cool.png', 'testing.png'),
        runzcord.File(some_fp, 'cool_filename.png'),
    ]
...
You can also get single elements a la :func:`runzcord.utils.find` or :func:`runzcord.utils.get` via
...
After: ::
    ...
    vc.play(runzcord.FFmpegPCMAudio('testing.mp3'), after=lambda e: print('done', e))
    vc.is_playing()
    vc.pause()
    vc.resume()
    vc.stop()
...
For example, you can add a :class:`PCMVolumeTransformer` to allow changing the volume: ::
    ...
    vc.source = runzcord.PCMVolumeTransformer(vc.source)
...
Usage is as simple as doing: ::
    ...
    client = runzcord.AutoShardedClient()
...
If you want more control over the sharding you can specify ``shard_count`` and ``shard_ids``. ::
    ...
    client = runzcord.AutoShardedClient(shard_count=10)

    # launch specific shard IDs in this process
    client = runzcord.AutoShardedClient(shard_count=10, shard_ids=(1, 2, 5, 6))
...
as the built-in "runzcord" converters.
