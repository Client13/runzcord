...
See `this page <https://guide.runzcord.dev/popular-topics/intents#why-do-most-messages-have-no-content>`_ of the guide for more information.
If you do not have access to the intent yet, you can temporarily continue using API v9 by calling ``runzcord.http._workaround_set_api_version(9)`` before connecting,
...
- Support ``List[str]`` and ``Dict[str, str]`` in ``option`` parameter of :class:`runzcord.ui.Select` (:issue:`326`)
...
- Fix shadowed ``runzcord.message`` module (:issue:`380`)
...
- Make ``runzcord.types.interactions`` importable at runtime (:issue:`493`)
...
- |commands| Add :class:`commands.Range <runzcord.ext.commands.Range>`, see :ref:`param_ranges` (:issue:`237`, :issue:`276`, :issue:`316`)
- |commands| Add :func:`Bot.load_extensions <runzcord.ext.commands.Bot.load_extensions>` and :func:`utils.search_directory` (:issue:`245`)
- |commands| Add :class:`commands.LargeInt <runzcord.ext.commands.LargeInt>` (:issue:`264`)
...
- Support :class:`runzcord.Attachment` options in slash commands (:issue:`226`)
...
- |commands| Fix typing of ``ctx`` parameter in :class:`~runzcord.ext.commands.Converter` (:issue:`292`)
...
- Fix :func:`tasks.loop <runzcord.ext.tasks.loop>` usage with fixed times (:issue:`337`)
...
- ``runzcord.types.ThreadArchiveDuration`` is now ``ThreadArchiveDurationLiteral``, to avoid confusion with the new :class:`ThreadArchiveDuration` enum
...
- |commands| Add parameter injections (`example <https://github.com/runzcordDev/runzcord/blob/master/examples/slash_commands/injections.py>`_) (:issue:`130`)
...
- Move documentation to https://docs.runzcord.dev/
...
- Fix :class:`Select.append_option <runzcord.ui.Select.append_option>` not raising an error if 25 options have already been added
...
- Ensure ``runzcord`` package is only run if executed as a script (:issue-dpy:`6483`)
...
- ``_`` and ``-`` characters are now stripped when making a new cog using the ``runzcord`` package (:issue-dpy:`6313`)
...
- |commands| :class:`MemberConverter <ext.commands.MemberConverter>` now properly lazily fetches members if not available from cache.
    - This is the same as having ``runzcord.Member`` as the type-hint.
...
- Make the output of ``python -m runzcord --version`` a bit more useful.
...
- New utility function, :func:`runzcord.utils.get` to simplify retrieval of items based on attributes.
...
- Add :class:`runzcord.Game` to represent any game with custom text to send to :meth:`Client.change_status`.
...
- :meth:`Client.logs_from` now handles cases where limit > 100 to sidestep the runzcord API limitation.
...
- Mentions are now triggered normally. This was changed due to the way runzcord handles it internally.
