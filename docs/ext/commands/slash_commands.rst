...
You may want to define a couple of options for your slash command. In runzcord, the definition of options is based on annotations.
...
- :class:`float`
- :class:`bool`
- :class:`runzcord.User` or :class:`runzcord.Member`
- :class:`runzcord.abc.GuildChannel`\*
- :class:`runzcord.Role`\*\*
- :class:`runzcord.Attachment`
...
.. code-block:: python3
    ...
    @bot.slash_command()
    async def command(
        inter: runzcord.ApplicationCommandInteraction,
...
.. note::
    ...
    .. code-block:: ini
        ...
        [mypy]
        plugins = runzcord.ext.mypy_plugin
...
Python has no truly *clean* way to provide metadata for parameters, so runzcord uses the same approach as fastapi using
parameter defaults. At the current time there's only :class:`~runzcord.ext.commands.Param`.
...
.. code-block:: python3
    ...
    @bot.slash_command()
    async def math(
        interaction: runzcord.ApplicationCommandInteraction,
...
.. note ::
    ...
    Converter classes are completely unusable in runzcord due to their inconsistent typing.
...
.. code-block:: python3
    ...
    @bot.slash_command()
    async def blep(inter: runzcord.ApplicationCommandInteraction, animal: Animal):
...
.. code-block:: python3
    ...
    async def autocomp_langs(inter: runzcord.ApplicationCommandInteraction, user_input: str):
...
.. code-block:: python3
    ...
    @bot.slash_command()
    async def languages(inter: runzcord.CommandInteraction, language: str):
        pass


    @languages.autocomplete("language")
    async def language_autocomp(inter: runzcord.CommandInteraction, string: str):
        string = string.lower()
...
.. code-block:: python3
    ...
    @show.sub_command()
    async def user(inter, user: runzcord.User):
        """
...
We have them, look at `this example <https://github.com/runzcordDev/runzcord/blob/master/examples/slash_commands/injections.py>`_ for more information ✨
...
.. code-block:: python3
    ...
    @bot.slash_command()
    async def add_5(inter: runzcord.ApplicationCommandInteraction, num: int):
        """
...
To load a directory or file containing localizations, use :func:`bot.i18n.load(path) <runzcord.LocalizationStore.load>`:
...
.. code-block:: python3
    ...
    async def language_autocomp(inter: runzcord.ApplicationCommandInteraction, user_input: str):
...
.. code-block:: python3
    ...
    async def command(inter: runzcord.ApplicationCommandInteraction):
        ...
...
.. code-block:: python3
    ...
    @bot.slash_command(default_member_permissions=runzcord.Permissions(manage_guild=True, moderate_members=True))
    async def command(inter: runzcord.ApplicationCommandInteraction):
        ...
...
.. code-block:: python3
    ...
    async def config(inter: runzcord.ApplicationCommandInteraction):
        ...
