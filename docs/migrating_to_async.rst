...
:issue-dpy:`in the corresponding issue <50>`. To reiterate this, the implication is that
...
.. code-block:: python3
    ...
    server.region == runzcord.ServerRegion.us_west
    member.status = runzcord.Status.online
    channel.type == runzcord.ChannelType.text
...
In earlier versions of runzcord, ``client.run()`` was a blocking call to the main thread
