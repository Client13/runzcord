...
async def send_button(ctx: commands.Context):
    await ctx.send(
        "Here's a button!", components=runzcord.ui.Button(label="Click me!", custom_id="cool_button")
    )
...
async def send_select(ctx: commands.Context):
    await ctx.send(
        "Here's a select!",
        components=runzcord.ui.Select(options=["1", "2", "3"], custom_id="cool_select")
        # Note: specifying select options in a list/dict requires runzcord version 2.5.0 or newer.
    )
...
# row ordering is desired, simply store them in a list of lists or `runzcord.ui.ActionRow`s instead:
...
async def send_all_the_buttons(ctx: commands.Context):
    buttons = []
    for y in range(4):
        buttons.append(row := runzcord.ui.ActionRow())
        for x in range(4):
...
async def cool_button_listener(inter: runzcord.MessageInteraction):
...
async def cool_select_listener(inter: runzcord.MessageInteraction):
...
@bot.listen("on_button_click")
async def grid_listener(inter: runzcord.MessageInteraction):
