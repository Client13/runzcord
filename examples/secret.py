...
def create_overwrites(ctx, *objects):
    ...
    A `runzcord.PermissionOverwrite` allows you to determine the permissions
    of an object, whether it be a `runzcord.Role` or a `runzcord.Member`.
    ...
    # for each `runzcord.Role` or `runzcord.Member`.
    overwrites = {obj: runzcord.PermissionOverwrite(view_channel=True) for obj in objects}

    # prevents the default role (@everyone) from viewing the channel
    # if it isn't already allowed to view the channel.
    overwrites.setdefault(ctx.guild.default_role, runzcord.PermissionOverwrite(view_channel=False))

    # makes sure the client is always allowed to view the channel.
    overwrites[ctx.guild.me] = runzcord.PermissionOverwrite(view_channel=True)
...
@commands.guild_only()
async def text(
    ctx: commands.GuildContext, name: str, *objects: typing.Union[runzcord.Role, runzcord.Member]
):
...
@secret.command()
@commands.guild_only()
async def emoji(ctx: commands.GuildContext, emoji: runzcord.PartialEmoji, *roles: runzcord.Role):
